 ...
 */
public class JpaLocalTxnInterceptor implements MethodInterceptor {
    private static final Logger LOGGER = LoggerFactory.getLogger(JpaLocalTxnInterceptor.class);

    private static final Pattern PATTERN = Pattern.compile("^(update|delete|save)");
    @Inject
    ...
    public Object invoke(MethodInvocation methodInvocation) throws Throwable {
        boolean hasTrans = PATTERN.matcher(methodInvocation.getMethod().getName()).find();
        if (hasTrans) {
            return transInvoke(methodInvocation);
        } else {
            return noTransInvoke(methodInvocation);
        }
    }

    public Object noTransInvoke(MethodInvocation invocation) throws Throwable {
    ...
    private Object transInvoke(MethodInvocation methodInvocation) throws Throwable {
